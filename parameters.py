...
def set_user(newuser):
    ...
    wnd.window.set_fullscreen(config.cfg.WINDOW_FULLSCREEN) # window size needs to be changed
    update_all_labels()
    save_last_user('defaults.ini')


def get_users():
    users = ['default'] + [fn.split('-')[0] for fn in os.listdir(utils.get_data_dir()) if '-stats.txt' in fn]
